...
impl Offset {
    pub fn convert(&mut self, map: &Vec<Option<usize>>) {
        self.begin = map
            ...
            .expect(
                format!(
                    "Bytes to unicode: Begin offset {} must exist in map",
                    self.begin
                )
                .as_str(),
            )
            .expect("Offset in map may not be None");
        self.end = map
            .get(self.end)
            .expect(
                format!(
                    "Bytes to unicode: End offset {} must exist in map",
                    self.end
                )
                .as_str(),
            )
...
impl<'a> Match<'a> {
    ...
    pub fn solution(&self) -> Option<&VariantResult> {
        if let Some(selected) = self.selected {
            self.variants
                .as_ref()
                .expect("match must have variants when 'selected' is set")
                .get(selected)
        } else {
            None
        }
...
pub fn find_boundaries<'a>(text: &'a str) -> Vec<Match<'a>> {
    ...
    for (i, c) in text.char_indices() {
        if let Some(b) = begin {
            if c.is_alphabetic() {
                //boundary ends here
                boundaries.push(Match::new_empty(&text[b..i], Offset { begin: b, end: i }));
                begin = None;
            }
        } else {
            if !c.is_alphabetic() {
    ...
    if let Some(b) = begin {
        ...
        boundaries.push(Match::new_empty(
            &text[b..],
            Offset {
                begin: b,
                end: text.len(),
...
pub fn redundant_match<'a>(candidate: &Match<'a>, matches: &[Match<'a>]) -> bool {
    for refmatch in matches.iter() {
        if refmatch.n == 1 {
            if refmatch.offset.begin >= candidate.offset.begin
                && refmatch.offset.end <= candidate.offset.end
            {
...
impl ContextRule {
    ...
    pub fn matches(
        &self,
        sequence: &[(VocabId, u32)],
        begin: usize,
        sequence_result: &mut Vec<Vec<PatternMatchResult>>,
    ) -> bool {
        ...
        for (cursor, contextmatch) in self.pattern.iter().enumerate() {
            if !sequence_result[begin + cursor].is_empty()
                || !contextmatch.matches(sequence, begin + cursor)
            {
                found = false;
                break;
            }
        ...
        if found {
            for cursor in 0..self.pattern.len() {
                sequence_result[begin + cursor] = if self.tag.is_empty() {
                    ...
                    self.tag
                        .iter()
                        .zip(self.tagoffset.iter())
                        .filter_map(|(tag, tagoffset)| {
                            if cursor as u8 >= tagoffset.0
                                && (cursor as u8) < tagoffset.0 + tagoffset.1
                            {
...
pub(crate) fn remap_offsets_to_unicodepoints<'a>(
    text: &'a str,
    mut matches: Vec<Match<'a>>,
) -> Vec<Match<'a>> {
